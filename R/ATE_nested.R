    ...
    for (r in 1:replications) {
      ...
      for (k in 1:K) {
        ...
        for (s in unique_S) {
          id_S <- id_by_S[[s]]
          part_S <- partition[[s]]
          test.id <- c(test.id, id_S[which(part_S == k %% K)])
          sm.id <- c(sm.id, id_S[which(part_S == (k + 1) %% K)])
          tm.id <- c(tm.id, id_S[which(part_S == (k + 2) %% K)])
          om.id <- c(om.id, id_S[which(part_S == (k + 3) %% K)])
        }
        X_test <- X[test.id, ]
        Y_test <- Y[test.id]
        A_test <- A[test.id]
